...
export function zeroPad(value: number | string, length = 2): string {
  ...
  const paddedNo = strNo.length >= length ? strNo : ([...Array(length)].map(() => '0').join('') + strNo).slice(length * -1);
 ...
 */
export function calcTimeDelta(date: Date | string | number, options: CountdownTimeDeltaOptions = {}): CountdownTimeDelta {
  ...
  const total = Math.round(parseFloat(((overtime ? timeLeft : Math.max(0, timeLeft)) / 1000).toFixed(clampedPrecision)) * 1000);
 ...
 */
export function formatTimeDelta(timeDelta: CountdownTimeDelta, options?: CountdownTimeDeltaFormatOptions): CountdownTimeDeltaFormatted {
