...
import { propsCompare } from './utils';
import { Context } from './ModalContext';
import { Handler } from './types';

const useModal = (modal: React.ReactNode, closeOnOverlayClick = true, updateOnPropsChange = false, modalId = 'defaultNodeId'): [Handler, Handler] => {
  ...
  useEffect(() => {
    ...
    if (updateOnPropsChange && isOpen && nodeId === modalId) {
      ...
      if (modalProps && oldModalProps && !propsCompare(modalProps, oldModalProps)) {
        setModalNode(modal);
      }
    }
