...
import { useBankSellRING, useBankUniswapTokenToEthInputPrice } from 'hooks/useBank';
...
const Sell: React.FC<Props> = ({ className }) => {
  ...
  const inputRINGValueString = parseUnits(inputRINGValueBn.toFixed(RING.decimals), RING.decimals);

  const { price, pay } = useBankUniswapTokenToEthInputPrice(landId, { address: bundleApi[landId].getAddressByName(extendLandId(landId), 'TOKEN_RING'), decimals: 18 }, inputRINGValueString.toString());

  const { handleUniswapSellRING, pendingTx } = useBankSellRING(landId);
  ...
  const handleSellRING = useCallback(
    (input: string) => {
      if (!input) {
      ...
      handleUniswapSellRING(value);
    },
    [handleUniswapSellRING, toastError]
  );
