...
import { useLandAskWithToken } from 'hooks/useLand';
...
const SellLandModal: React.FC<Props> = ({ title, onDismiss, tokenId, landId, ...props }) => {
  ...
  const { handleLandAskWithToken, pendingTx } = useLandAskWithToken(landId);

  const confirmSellLand = useCallback(() => {
    if (!startPrice) {
    ...
    handleLandAskWithToken(tokenId, ethersStartPrice, ethersEndPrice, day2Seconds(duration).toString(), () => {
      onDismiss();
    });
  }, [RING.decimals, RING.symbol, duration, endPrice, handleLandAskWithToken, onDismiss, startPrice, toastError, tokenId]);

  return (
    <Modal title={title} onDismiss={onDismiss} {...props}>
      <StyledForm flexDirection='column'>
        <Form layout='vertical'>
          <Form.Item label={t('Starting')} required tooltip={`${t('No less than')} 1 ${RING.symbol}`}>
            <Input placeholder={`${t('No less than')} 1 ${RING.symbol}`} suffix={RING.symbol} onChange={onChangeStartPrice} />
          </Form.Item>
          <Form.Item label={t('Ending')} required tooltip={`${t('No less than')} 1 ${RING.symbol}`}>
            <Input placeholder={`${t('No less than')} 1 ${RING.symbol}`} suffix={RING.symbol} onChange={onChangeEndPrice} />
          </Form.Item>
          <Form.Item label={t('Duration')} required tooltip={`${t('Greater than')} 0.1 ${t('Days')}`}>
            <Input placeholder={`${t('Greater than')} 0.1 ${t('days')}`} suffix={t('Days')} onChange={onChangeDuration} />
          </Form.Item>
        </Form>
      </StyledForm>
      <Button scale='sm' onClick={confirmSellLand} isLoading={pendingTx}>
        {t('Confirm')}
      </Button>
    </Modal>
  );
...
export default React.memo<Props>(styled(SellLandModal)``);
